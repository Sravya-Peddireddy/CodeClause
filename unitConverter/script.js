function getElementById() {
  ...
  function area() {
    ...
    var ip = document.getElementById('aip').value;
...
function temp() {
    var from = parseInt(document.getElementById('ftemp').value);
    var to = parseInt(document.getElementById('ttemp').value);
    var ip = parseFloat(document.getElementById('tip').value);
    var totext = document.getElementById('ttemp').options[document.getElementById('ttemp').selectedIndex].text;
  
    if (isNaN(ip) || ip === '') {
      document.getElementById('top').value = 'Please enter a value';
      return;
    }
    ...
    if (from === 1 && to === 1) {
      ...
      result = ((ip - 32) * 5 / 9) + 273.15;
    } else if (from === 3 && to === 1) {
      result = ip - 273.15;
    } else if (from === 3 && to === 2) {
      result = (ip - 273.15) * 9 / 5 + 32;
    } else if (from === 3 && to === 3) {
      result = ip;
    }
  
    document.getElementById('top').value = result.toFixed(2) + ' ' + totext;
  }
...
function selunit() {
    ...
    if (document.getElementById('ut').value == '2') {
      hi();
      uc2();
      area();  // call area() function when "Area" is selected
    }
